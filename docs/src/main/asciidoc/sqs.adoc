...
Without Spring Boot, it is necessary to import the `SqsBootstrapConfiguration` class in a `@Configuration`, as well as declare a `SqsMessageListenerContainerFactory` bean.
...
When messages larger than the SQS size limit (256 KB, or 1 MiB with the newer limit) need to be sent, Spring Cloud AWS provides integration with the https://github.com/awslabs/amazon-sqs-java-extended-client-lib[Amazon SQS Extended Client Library] which transparently offloads large message payloads (up to 2 GB) to Amazon S3.

When the extended client library is on the classpath and the `spring.cloud.aws.sqs.extended.bucket` property is set, the autoconfigured `SqsAsyncClient` is automatically wrapped with `AmazonSQSExtendedAsyncClient`. This means both `SqsTemplate` and `@SqsListener` transparently handle large payloads without any code changes.
...
The version is managed by the Spring Cloud AWS BOM.

==== Configuration

[source,properties]
----
spring.cloud.aws.sqs.extended.bucket=my-payload-bucket
----
...
| `spring.cloud.aws.sqs.extended.bucket`
| (required)
| S3 bucket name for storing large payloads. Setting this property activates the extended client.

| `spring.cloud.aws.sqs.extended.cleanup-s3-payload`
| `true`
| Whether to delete the S3 payload object when the SQS message is deleted.

| `spring.cloud.aws.sqs.extended.use-legacy-reserved-attribute-name`
| `true`
| Whether to use the legacy `SQSLargePayloadSize` attribute name. Set to `false` for the current attribute name.

| `spring.cloud.aws.sqs.extended.ignore-payload-not-found`
| `false`
| Whether to silently ignore messages whose payload is not found in S3.

| `spring.cloud.aws.sqs.extended.s3-key-prefix`
| `""`
| Prefix prepended to S3 keys under which payloads are stored.
|===

NOTE: The extended client requires an `S3AsyncClient` bean in the application context. If you have the `spring-cloud-aws-starter-s3` starter on the classpath, this is autoconfigured automatically.

NOTE: Messages smaller than 256 KB are sent directly to SQS by default. The extended client only offloads payloads exceeding the SQS size limit.
...
[[sqs-template-send]]
==== SqsTemplate
When using Spring Boot and auto-configuration, a `SqsTemplate` instance is autowired by default if no other template bean is found in the context.
...
If only sync or async operations are to be used, the corresponding interface can be used to narrow down the available methods.
...
If more complex configuration is required, a builder is also provided along with a set of options:
...
Note that messages sent with the `SqsTemplate` by default contain a header with the type information, so no configuration is needed.
...
If there are messages that failed to be sent within a batch, corresponding `SendResult.Failed` objects are generated.
...
By default, if there is at least one failed message in a send batch operation, a `SendBatchOperationFailedException` will be thrown.
...
If type mapping information is not available, the payload class can be specified either in the <<template-options>> or in the `receive()` method variants:
...
[[sqs-compatible-brokers-support]]
==== SQS-compatible brokers support
...
* **Send side**: If the send response contains a non-UUID message ID,
  ...
  in `SendResult.additionalInformation()` under the `SqsTemplateParameters.SQS_RAW_MESSAGE_ID_PARAMETER_NAME` key.
...
Both interfaces come with `single message` and `batch` methods.
...
The `MessageListenerContainer` manages the entire message lifecycle, from polling, to processing, to acknowledging.
...
IMPORTANT: If you use this method for setting the `SqsAsyncClient` instance in the factory, all containers created by this factory will share the same `SqsAsyncClient` instance.
...
One or more queues can be specified in the annotation through the `queueNames` or `value` properties — there is no distinction between the two.
...
===== Handling different payloads in the same listener

It is possible to handle different payloads in the same listener by annotating handler methods with the `@SqsHandler` annotation.
...
The `isDefault = true` parameter designates a method as the fallback handler for messages that do not match any other handler's parameter type.
...
Since 3.1.1, when receiving SNS messages through the `@SqsListener`, the message includes all attributes of the `SnsNotification`. To receive only the `Message` part of the payload, use the `@SnsNotificationMessage` annotation.
...
The following is a sample with many arguments:
...
IMPORTANT: If the same factory is shared by both delivery methods, any supplied `ErrorHandler`, `MessageInterceptor`, or `MessageListener` should implement the appropriate methods.
...
| `spring.cloud.aws.sqs.endpoint` | Configures the endpoint used by `SqsAsyncClient`. | No | `http://localhost:4566`
| `spring.cloud.aws.sqs.region` | Configures the region used by `SqsAsyncClient`. | No | `eu-west-1`
...
For batching acknowledgements, a message is considered as no longer inflight when it is handed to the acknowledgement queue.
See <<Acknowledging messages>>.

|<<maxMessagesPerPoll>>
|1 - `Integer.MAX_VALUE`
|10
|The maximum number of messages that will be received by a poll to an SQS queue in this container.
...
|1 - 10 seconds
|10 seconds
|The maximum duration for a poll to an SQS queue before returning empty.
...
|true, false
|true
|Determines whether the container should start automatically. When set to `false`, the
...
This value is overridden by `@SqsListener` depending on whether the listener method contains a `List` argument.
...
The containers themselves are not Spring-managed beans, and the registry is responsible for managing these containers' lifecycle during application startup and shutdown.
...
Providing queue URLs instead of names and not requesting queue attributes can result in slightly better startup times since there is no need for such requests.
...
This configuration is available in the `SqsContainerOptions` `queueNotFoundStrategy`.
...
In these cases, the container's lifecycle will be managed by the Spring context at application startup and shutdown.
...
NOTE: The `SqsMessageListenerContainer.builder()` allows specifying the `SmartLifecycle.phase` to override the default value defined in `MessageListenerContainer.DEFAULT_PHASE`.

===== Retrieving containers from the registry

Containers can be retrieved by fetching the `MessageListenerContainerRegistry` bean from the context and using the `getListenerContainers` and `getContainerById` methods.
...
`FIFO` SQS queues are fully supported for receiving messages — queues with names that end in `.fifo` will automatically be set up as such.
...
IMPORTANT: If a different message is returned, it is important to set the `SqsHeaders.SQS_RECEIPT_HANDLE_HEADER` to the receipt handle of the original message so that the original message is acknowledged after processing.
Also, a `SqsHeaders.SQS_MESSAGE_ID_HEADER` must always be present.

IMPORTANT: The `intercept` methods must not return null.

The `afterProcessing` methods are executed after a message is processed and the `ErrorHandler` is invoked, but before the message is acknowledged.
...
IMPORTANT: If the message should not be acknowledged and the `ON_SUCCESS` acknowledgement mode is set, it is important to propagate the error.
...
To enable half jitter, set `jitter(Jitter.HALF)` on the builder:
...
For listener methods inherited from generic superclasses, type variables are resolved against the concrete listener bean class.
Because the conversion hint is the listener `MethodParameter`, smart converters can also use other parameter metadata, such as Jackson's `@JsonView`.
...
- `MANUAL` - The framework will not acknowledge messages automatically and `Acknowledgement` objects can be received in the listener method.
...
Setting `acknowledgementThreshold` to `0` will disable acknowledging per number of messages, and messages will be acknowledged only on the specified `acknowledgementInterval`.
...
Manual acknowledgement can be used in conjunction with acknowledgement batching — the message will be queued for acknowledgement but will not be executed until one of the acknowledgement thresholds is reached.
...
NOTE: When using acknowledgement batching, a message is considered as no longer inflight when it is delivered to the acknowledgement queue. In this case, the actual number of inflight messages on the AWS SQS console can be higher than the configured value.
When using immediate acknowledgement, a message is considered as no longer inflight after it has been acknowledged or throws an error.


===== maxMessagesPerPoll
Set in `SqsContainerOptions` or the `@SqsListener` annotation.
Represents the maximum number of messages returned by a single poll to an SQS queue, to a maximum of 10.
This value has to be less than or equal to `maxConcurrentMessages`.
Defaults to 10.

Note that even if the queue has more messages, a poll can return fewer messages than specified. See the AWS documentation for more information.
...
This wait is applied per queue, and one queue does not interfere with another in this regard.
Defaults to 10 seconds.

===== pollBackOffPolicy
Since 3.2, it is possible to specify a `BackOffPolicy` that will be applied when a polling thread throws an exception.
...
After `maxDelayBetweenPolls`, if `maxMessagesPerPoll` permits are not available, the container will poll for the difference — that is, as many messages as have been processed so far, if any.

For example, consider a scenario where the container is configured for `maxConcurrentMessages` = 20, `maxMessagesPerPoll` = 10, `maxDelayBetweenPolls` = 5 seconds, and `pollTimeout` = 10 seconds.

The container starts in `low throughput mode`, meaning it will attempt a single poll for 10 messages.
If any messages are returned, it will switch to `high throughput mode` and make up to two simultaneous polls for 10 messages each.
If all 20 messages are retrieved, it will not attempt any more polls until messages are processed.
If after 5 seconds for `maxDelayBetweenPolls` six messages have been processed, the framework will poll for those six messages.
If the queue is depleted and a poll returns no messages, it will enter `low throughput` mode again and perform only one poll at a time.
...
If necessary, multiple `BackPressureHandler` instances can be combined by using the `CompositeBackPressureHandler`.
Each of the `BackPressureHandler` delegates is chained in the order they are provided.
...
For example, to implement the `BackPressureMode.ALWAYS_POLL_MAX_MESSAGES` strategy, you can combine a concurrency limiter, an adaptive throughput handler, and a "full batch only" handler.
...
- `FullBatchBackPressureHandler`: Ensures polls will always be done with a full batch of messages, meaning that the number of messages polled will always be equal to `maxMessagesPerPoll` if possible or `0` if not possible.
...
When a `MessageListener`, `MessageInterceptor`, and `ErrorHandler` implementation is set to a `MessageListenerContainer` or `MessageListenerContainerFactory`, these are adapted by the framework. This way, blocking and non-blocking components can be used in conjunction with each other.
...
NOTE: To achieve higher throughput, it is encouraged that, at least for simpler logic in message listeners, `interceptors`, and `error handlers`, the async variants are used.
...
If by the time the execution reaches a `blocking` component it is already on a framework thread, it remains in the same thread to avoid excessive thread allocation and hopping.

IMPORTANT: When using `async` methods, it is critical not to block the incoming thread, as this can be very detrimental to overall performance.
...
When providing a custom executor, it is important that it is configured to support all threads that will be created, which should be `maxConcurrentMessages * total number of queues`.
...
There can be multiple `SqsAsyncClientCustomizer` beans present in a single application context. The `@Order(..)` annotation can be used to define the order of execution.
...
2. Having an `ObservationRegistry` bean in your application context
...
To add custom tags while preserving all default tags, the `DefaultConvention` classes can be extended and the `getCustomLowCardinalityKeyValues` and/or `getCustomHighCardinalityKeyValues` methods overridden:
...
=== IAM permissions
The following IAM permissions are required by Spring Cloud AWS SQS:
...
| To use `@SqsListener` with `SqsMessageListenerContainerFactory`, the following permission is also required
...
The `SqsMessageHandler` is for publishing a single message (or a batch of messages) to an SQS queue configured explicitly on the `SqsMessageHandler`, or resolved via a SpEL expression against the request message.
...
When the `SqsMessageHandler` is set to `async` mode, the result of the send operation is produced as a reply message into the `outputChannel`.
For a single request, the reply message is created based on the `SendResult`.
With the request message payload as a `Collection<Message<?>>`, the `SqsAsyncOperations.sendManyAsync()` is performed; the `SendResult.Batch` is produced as-is as the payload of the reply message.
...
If the `ListenerMode` on the `SqsContainerOptions` is set to `BATCH`, the received `Collection<Message<?>>` is wrapped into a single message before being produced.
...
The Spring Integration dependency in the `spring-cloud-aws-sqs` module is `optional` to avoid unnecessary artifacts on the classpath when Spring Integration is not used.
